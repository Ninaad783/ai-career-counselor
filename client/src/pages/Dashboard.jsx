...
const Dashboard = () => {
  ...
  const handleDeleteRoadmap = async (e, roadmapId) => {
    ...
    if (!window.confirm("Are you sure you want to remove this saved roadmap track?")) return;

    try {
      const res = await API.delete(`/learning/delete/${roadmapId}`);
      if (res.data.success) {
        // Cleanly slice out the item from state array to update UI without browser refreshes
        setSavedRoadmaps((prev) => prev.filter((map) => map._id !== roadmapId));
